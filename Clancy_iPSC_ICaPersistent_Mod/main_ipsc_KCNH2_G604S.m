% Main File to generate KCNH2 p.G604S iPSC-CM Models
close all; clear; clc
...
g_K1_index = 1;   % Conductance of IK1
...
elseif model_to_run == 3
    ...
    % --- persistent non-inactivating ICa,L LOGIC ---
...
% Overwrite the original variable with modified one for the simulation
...
for i= 1:size(values,1)
    ...
    Istim(i) = update_step_i(16); 
end
%% Calcium Flux analysis and Calcium Transient Trace

%ca_analysis_KCNH2( Time, Iup, INaCa, IpCa, Cai, plot_title )
%% action potential trace for baseline model
...
[ca_results, validation_data] = ca_analysis_KCNH2( Time, Iup, INaCa, IpCa, Cai, plot_title );
%% ========================================================================
...
% --- Display Calcium and Flux Tables (from ca_analysis_KCNH2) ---
...
%%
    ...
    % Assumes the 'time' vector is in microseconds (µs).

    % --- Define a search window for the AP peak (in microseconds) ---
    ...
    % --- Search for the AP peak ONLY after the upstroke ---
    ...
    % --- Find the true AP Start time (t_AP_start) using RMP + 1mV ---
    ...
    for i = 1:length(apd_levels)
        ...
        if ~isempty(idx2_repol) && idx2_repol > 1
            ...
            % --- Store results with unit conversion ---
    ...
    % --- Store other results with unit conversion ---
    results.RMP = RMP;
    results.APA = APA;
    results.V_max = V_max;
...
function DisplayAPMorphology(time, voltage, results)
    ...
    for i = 1:length(apd_levels)
        ...
        if ~isnan(t_rep_val)
            ...
            % --- Plot horizontal line from t_depol to t_rep ---
    ...
    % Mark the AP start time (t_AP_start) with a red dot
